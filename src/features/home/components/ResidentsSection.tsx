...
export const ResidentsSection = ({ residents, activeResident, onBack, events = [] }: ResidentsSectionProps) => {
  if (activeResident) {
      ...
      <div className={styles.residentDetail}>
        ...
        <div className={styles.rdLinks}>
          {activeResident.instagramUrl && (
            <a href={activeResident.instagramUrl} className={styles.resLink} target="_blank" rel="noopener">Instagram</a>
          )}
          {activeResident.soundcloudUrl && (
            <a href={activeResident.soundcloudUrl} className={styles.resLink} target="_blank" rel="noopener">SoundCloud</a>
          )}
        </div>
        ...
        {residentEvents.length > 0 && (
          <div style={{ marginTop: '28px', paddingTop: '24px', borderTop: '1px solid var(--line)' }}>
            <div style={{ fontSize: '8px', letterSpacing: '.25em', textTransform: 'uppercase', color: 'rgba(220, 220, 220, .3)', marginBottom: '16px' }}>
